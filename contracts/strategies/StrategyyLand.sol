...
contract StrategyYfii {
    ...
    address constant public yLand = address(0x9e3f692d1d95B112dcC5261d10EDCc2FF6886146);
    ...
    function deposit() external {
        ...
        yLand(pool).stake(IERC20(want).balanceOf(address(this)));
    }
    ...
    function harvest() public {
        yLand(pool).getReward();
        IERC20(yLand).safeApprove(balancer, 0);
        IERC20(yLand).safeApprove(balancer, IERC20(yLand).balanceOf(address(this)));
        Balancer(balancer).swapExactAmountIn(yLand, IERC20(yLand).balanceOf(address(this)), dai, 0, uint(-1));
    ...
    function balanceOfYfii() public view returns (uint) {
        return yLand(pool).balanceOf(address(this));
    }
